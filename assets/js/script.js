...
// Service tabs (Signage + Branding)
(function () {
  function initServiceTabs(root) {
    const tabs = Array.from(root.querySelectorAll(".service-one_title[data-tab]"));
    const panels = Array.from(root.querySelectorAll(".service-one_image[data-panel]"));

    if (!tabs.length || !panels.length) return;

    function setActive(index) {
      tabs.forEach((t) => {
        const isActive = String(t.getAttribute("data-tab")) === String(index);
        t.classList.toggle("active", isActive);
        t.setAttribute("aria-selected", isActive ? "true" : "false");
      });

      panels.forEach((p) => {
        const isActive = String(p.getAttribute("data-panel")) === String(index);
    ...
    const initial = (tabs.find((t) => t.classList.contains("active")) || tabs[0]).getAttribute("data-tab");
    setActive(initial);
  }

  document.addEventListener("DOMContentLoaded", function () {
    document.querySelectorAll("[data-service-tabs]").forEach(initServiceTabs);
  });
})();